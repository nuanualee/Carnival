...
public class PlayContempSongActivity extends AppCompatActivity {
                ...
                popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    ...
                    public boolean onMenuItemClick(MenuItem item) {//SECOND ONCLICK
                        ...
                        if (id == R.id.addToPlaylist) {
                            Intent intent = new Intent(PlayContempSongActivity.this, AddToPlaylist.class);
                            intent.putExtra("TITLE", title);
                            startActivity(intent);
                        } else if (id == R.id.addToMoments) {
                            Intent intent2 = new Intent(PlayContempSongActivity.this, NewMomentsActivity.class);
    ...
    public void displaySongBasedOnIndex(int selectedIndex) {
        ...
        title = contempSong.getTitle();
        Log.d("temasek", "the title is: " + title);
        artiste = contempSong.getArtiste();
        filelink = contempSong.getFileLink();
        drawable = contempSong.getDrawable();
    ...
    private void gracefullyStopsWhenMusicEnds() { //new method to allow music player to go back to PLAY
        player.setOnCompletionListener(new MediaPlayer.OnCompletionListener() { //goig to listen: when it completed played
            ...
            public void onCompletion(MediaPlayer mp) { //ON COMPLETION, finished playing
                Toast.makeText(PlayContempSongActivity.this, "Song ended", Toast.LENGTH_SHORT).show();

                if (repeatFlag){
                    playOrPauseSong(null);

                    //init dialog
                    dialogBeeRepeat = new Dialog(PlayContempSongActivity.this);
                    //show dialog
